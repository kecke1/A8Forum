...
{
    [Authorize]
    public class ForumChallengesController : Controller
    {
        private readonly IMasterDataService _masterDataService;
        private readonly IForumChallengeService _forumChallengeService;

        public ForumChallengesController(IMasterDataService masterDataService,
        ...
        public async Task<IActionResult> Index(string? trackId = null, DateTime? InsertDateFrom = null, DateTime? InsertDateTo = null)
        {
            ...
            var query = (await _forumChallengeService
        ...
        public async Task<IActionResult> Details(string? id)
        {
            ...
            var challenge = await _forumChallengeService.GetForumChallengeAsync(id);
            if (challenge == null)
            {
                return NotFound();
            }
        ...
        {
            var q = (await _masterDataService.GetSeasonsAsync())
        ...
        {
            var q = (await _masterDataService.GetTracksAsync())
        ...
        public async Task<IActionResult> Create([Bind("StartDate,EndDate,Rules,Post,MaxRank, Track,Season,CustomTitle")] ViewModels.ForumChallengeViewModel challenge)
        {
            if (ModelState.IsValid)
            {
                await _forumChallengeService.AddForumChallengeAsync(challenge.ToDto());
        ...
        [ValidateAntiForgeryToken]
        [Authorize(Policy = "ForumChallengeAdminRole")]
        public async Task<IActionResult> Edit(string id, [Bind("ForumChallengeId,StartDate,EndDate,Rules,Post,MaxRank, Track, Season,CustomTitle")] ViewModels.ForumChallengeViewModel challenge)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    await _forumChallengeService.UpdateForumChallengeAsync(challenge.ToDto());
                }
                catch (Exception)
                {
                    return NotFound();
        ...
        {
            _forumChallengeService.DeleteForumChallengeAsync(id);
            return RedirectToAction(nameof(Index));
        }
    }
}