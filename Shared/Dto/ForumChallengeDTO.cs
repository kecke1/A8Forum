...
public class ForumChallengeDTO
{
    ...
    public required string MaxRank { get; set; }
    public required TrackDTO Track { get; set; }
    public required SeasonDTO Season { get; set; }
    public string? Leaderboard { get; set; }

    public string TitleHtml =>
        $"[p style='margin:0px;'][span style=\"white-space: nowrap;\"]{StartDate:dd.MM.yyyy}-{EndDate:dd.MM.yyyy}[/span][/p][p style='margin:0px;'][span style=\"white-space: nowrap;\"]{Track.TrackName} {Season.SeasonName}[/span][/p][p style='margin:0px;'][span style=\"white-space: nowrap;\"] {(!string.IsNullOrEmpty(CustomTitle) ? CustomTitle : $"max rank {MaxRank}")}[/span][/p]";
