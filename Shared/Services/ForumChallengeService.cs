...
public class ForumChallengeService(IRepository<ForumChallenge> forumChallengeRepository,
    ...
    : IForumChallengeService
{
    ...
    {
        await seriesRepository.CreateAsync(s.ToSeriesEntity());
    }
    ...
    {
        var c = await forumChallengeRepository.GetAsync(forumChallengeId);
        var season = await masterDataService.GetSeasonAsync(c.SeasonId);
        var track = await masterDataService.GetTrackAsync(c.TrackId);
    ...
    {
        var table =
            $@"[b]Leaderboard[/b][div style=""overflow:auto;""][table style='border-collapse: collapse;font-family: arial, sans-serif;']
...
[/div]
    ...
    public async Task<IEnumerable<ForumChallengeDTO>> GetForumChallengesAsync(DateTime? fromDate = null,
        DateTime? toDate = null)
    {
        var c = (await forumChallengeRepository
                .GetAsync(x =>
                    (fromDate == null || x.StartDate >= fromDate) &&
                    (toDate == null || x.StartDate <= (toDate ?? DateTime.MinValue).AddDays(1))))
            .ToList();

        var s = (await masterDataService.GetSeasonsAsync()).ToList();
        var t = (await masterDataService.GetTracksAsync()).ToList();
    ...
    {
        var r = await forumChallengeRunRepository.GetAsync(runId);
        var c = await GetForumChallengeAsync(r.ForumChallengeId);
        var m = await masterDataService.GetMemberAsync(r.MemberId);
        var v = await masterDataService.GetVehicleAsync(r.VehicleId);
    ...
    {
        var s = (await seriesRepository
                .GetAsync(x =>
                    x.StartDate >= (fromDate ?? DateTime.Now.AddYears(-1)) && x.StartDate <= (toDate ?? DateTime.Now)))
            .ToList();
    ...
    {
        return (await seriesRepository.GetAsync(seriesId)).ToDto();
    }
    ...
    public async Task<string> GetSeriesLeaderboardAsync(string seriesId)
    {
        ...
        var headers = challengesLb
            ...
            .Select(x => new
            {
                EndDate = x.Max(y => y.ChallengeEndDate), ForumChallengeUrl = x.Max(y => y.ForumChallengeUrl),
                ForumChallengeTitleHtml = x.Max(y => y.ForumChallengeTitleHtml)
            });

        var numberOfRuns = Convert.ToInt32(Math.Floor(Convert.ToDecimal(headers.Count()) / 2));
        var serieslLb = GetSeriesLeaderBoardForChallenges(challengesLb, numberOfRuns > 4 ? numberOfRuns : 4)
            .OrderBy(x => x.Position);
...
[/tbody][/table][/div]";
    ...
    {
        await seriesRepository.UpdateAsync(s.ToSeriesEntity());
    }
...
[/tr]";
    ...
    {
        return text.GetTdCell("padding:1px;padding-left:7px");
    }

    private async Task<IEnumerable<ForumChallengeRunDTO>> GetForumChallengeRunsAsync(ForumChallenge c)
    {
        var r = (await forumChallengeRunRepository
                .GetAsync(x => x.ForumChallengeId == c.Id))
            .ToList();

        var v = (await masterDataService.GetVehiclesAsync()).ToList();
        var m = (await masterDataService.GetMembersAsync()).ToList();
        var t = await masterDataService.GetTrackAsync(c.TrackId);
        var s = await masterDataService.GetSeasonAsync(c.SeasonId);
    ...
    private async Task<IEnumerable<ForumChallengeLeaderboardDto>> GetChallengeLeaderboardsByEndDate(DateTime startDate,
        DateTime endDate)
    {
        var c = (await forumChallengeRepository
                .GetAsync(
                    x => !x.Deleted && x.EndDate >= startDate && x.EndDate <= endDate && x.EndDate <= DateTime.Now))
            .ToList()
...
[td style=""border: 1px solid #dddddd; padding: 3px;font-weight:bold;""]{SpanString(x.Points.ToString())}[/td]{GetHtmlResultsForMember(x.MemberId, challenges)}[/tr]");
    ...
    private string GetHtmlResultsForMember(string memberId, IEnumerable<ForumChallengeLeaderboardDto> challenges)
    {
        ...
        foreach (var c in challenges.OrderByDescending(x => x.ChallengeEndDate).Select(x => x.ForumChallengeId)
                     .Distinct())
        {
